...
from src.config.env_config import load_env_config
...
class SceneManager:
    def __init__(self, screen_width: int, screen_height: int):
        ...
        # Check for development start scene override
        env_config = load_env_config()
        start_scene = None
        if "dev" in env_config and "start_scene" in env_config["dev"]:
            start_scene = env_config["dev"]["start_scene"]
            
        # Set initial scene
        if start_scene and start_scene in self.scenes:
            self.current_scene = self.scenes[start_scene]
            logger.info(f"Starting with development scene: {start_scene}")
        else:
            self.current_scene = self.scenes[SCENE_TITLE]

        # Start appropriate music
        if start_scene and start_scene in self.scenes:
            self._handle_music_transition(start_scene)
        else:
    ...
    def _handle_music_transition(self, scene_name: str):
        ...
        music_map = {
            ...
            SCENE_DRIVE_GAME: "drive_theme.ogg",
        }
