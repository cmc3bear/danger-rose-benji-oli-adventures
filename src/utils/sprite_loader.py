...
def load_vehicle_sprite(vehicle_type: str, base_path: str = None) -> pygame.Surface:
    """Load a vehicle sprite for The Drive minigame.
    
    Args:
        vehicle_type: Type of vehicle (professional, kids_drawing)
        base_path: Base path to vehicle assets (auto-detected if None)
        
    Returns:
        Loaded vehicle sprite surface
    """
    if base_path is None:
        # Auto-detect base path relative to this file
        current_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(os.path.dirname(current_dir))
        base_path = os.path.join(
            project_root, "assets", "images", "vehicles"
        )
    
    sprite_path = os.path.join(base_path, f"ev_{vehicle_type}.png")
    
    # Load sprite with original dimensions (128x192)
    return load_image(sprite_path, scale=None)


def get_vehicle_sprite_path(vehicle_type: str) -> str:
    """Get the path to a vehicle sprite.
    
    Args:
        vehicle_type: Type of vehicle (professional, kids_drawing)
        
    Returns:
        Absolute path to the vehicle sprite
    """
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(current_dir))
    return os.path.join(
        project_root, "assets", "images", "vehicles", f"ev_{vehicle_type}.png"
    )


# Cache for loaded vehicle sprites
